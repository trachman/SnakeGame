...
def game_window(width,height,cell_size):
	master = tk.Tk()
	master.geometry('{}x{}'.format(width+20,height+20))
...
def draw_grid(width, height, cell_size, canv):
	...
	for line in range(1,num_cells):
		y = line * cell_size
		canv.create_line(0,y,500,y)
	for line in range(1,num_cells):
		x = line * cell_size
		canv.create_line(x,0,x,500)

def clear_canvas(canv, snake, candy):
	for item in snake.ids:
		canv.delete(item)
...
def draw(snake, candy, canv):
	if candy.found:
		candy = Candy()
		snake.grow()
	if snake.direction == 'UP': snake.up()
	if snake.direction == 'DOWN': snake.down()
	if snake.direction == 'LEFT': snake.left()
	if snake.direction == 'RIGHT': snake.right()
	candy.id = canv.create_rectangle(candy.x, candy.y,candy.x+20, candy.y+20,fill='purple')
	for i in range(len(snake.blocks)):
		x = snake.blocks[i].x
		y = snake.blocks[i].y
		snake.ids.append(
			canv.create_rectangle(x,y,x+20,y+20,fill='red')
		)
	return snake, candy 
